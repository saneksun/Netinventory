...
def home(request):
 ...
 #       hostname = str(re.findall(r'(?<=hostname\/).*', hostname)[0])
        hostname = str(re.findall(r'([^\/]+$)', hostname)[0])
...
def modules(request, description):
 ...
 # provides search by selected appropriate urls on main page
    request.session['description'] = str(re.findall(r'([^\/]+$)', description)[0])
...
def nodedetails(request, hostname):
  # provides per-node details output from Nodes page
    request.session['hostname'] = hostname
    return redirect(home)
